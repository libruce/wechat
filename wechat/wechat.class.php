...
class Wechat {
  ...
  const GROUP_MEMBER_DELETE_URL = '/groups/delete?';
...
<Nonce><![CDATA[%s]]></Nonce>
  ...
  public function uploadForeverArticles($data) {
    ...
    dpm($data);
    $result = $this->http_post(self::API_URL_PREFIX . self::MEDIA_FOREVER_NEWS_UPLOAD_URL . 'access_token=' . $this->access_token, $data);
    dpm($result);
    if ($result) {
      $json = json_decode($result, TRUE);
      if (!$json || !empty($json['errcode'])) {
  ...
  public function deleteGroupById($groupid) {
    ...
    $url = self::API_URL_PREFIX . self::GROUP_MEMBER_DELETE_URL . 'access_token=' . $this->access_token;
    //dpm($url);
    $data = self::json_encode($data);
    //dpm($data);
