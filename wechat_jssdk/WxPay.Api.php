...
class WxPayApi {
  ...
  public static function unifiedOrder($inputObj, $timeOut = 6) {
    ...
    if ($inputObj->GetTrade_type() == "JSAPI" && !($inputObj->IsOpenidSet() || $inputObj->IsSubOpenidSet())) {
      throw new WxPayException("统一支付接口中，openid或sub_openid至少需要传一个！");
    }
    if ($inputObj->GetTrade_type() == "JSAPI" && $inputObj->IsSubOpenidSet() && !$inputObj->IsSubAppidSet()) {
      throw new WxPayException("使用sub_openid时必须传入sub_appid!");
    }
...
//    $inputObj->SetSub_Mch_id(WxPayConfig::GetSubMchId());
  ...
  public static function orderQuery($inputObj, $timeOut = 6) {
    ...
    //检查子商户号是否填写，如没有填写则使用当前登录用户的子商户id
    if (!$inputObj->IsSubMch_idSet()) {
      $inputObj->SetSub_mch_id(WxPayConfig::GetSubMchId());
    }
  ...
  public static function notify($callback, &$msg) {
    //获取通知的数据
    $xml = isset($GLOBALS['HTTP_RAW_POST_DATA']) ? $GLOBALS['HTTP_RAW_POST_DATA'] : FALSE;
    //如果返回成功则验证签名
    try {
  ...
  private static function reportCostTime($url, $startTimeStamp, $data) {
    //如果不需要上报数据
    if (WxPayConfig::GetReportLevel() == 0) {
      return;
    }
    //如果仅失败上报
    if (WxPayConfig::GetReportLevel() == 1 &&
  ...
  private static function postXmlCurl($xml, $url, $useCert = FALSE, $second = 30) {
    ...
    //如果有配置代理这里就设置代理
    if (WxPayConfig::GetCurlProxyHost() != "0.0.0.0"
      && WxPayConfig::GetCurlProxyPort() != 0
    ) {
